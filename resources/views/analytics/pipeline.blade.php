...
<div class="space-y-6 font-sans">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <div class="cc-card rounded-2xl shadow-sm border border-white/8 p-5">
            <div class="flex items-center justify-between mb-3">
                <span class="material-symbols-outlined text-{{ $color }}-600">{{ $icon }}</span>
                <span class="text-2xl font-extrabold text-slate-100">{{ $counts[$stage] ?? 0 }}</span>
            </div>
            <p class="text-xs font-bold text-slate-500 capitalize">{{ $stage }}</p>
            <p class="text-[11px] text-slate-500 mt-1">{{ \App\Helpers\FormatHelper::formatIDR($data->total_value ?? 0) }}</p>
        </div>
        @endforeach
    </div>
