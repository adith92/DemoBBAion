...
<div class="space-y-6 font-sans">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-cc-muted">Periode {{ $now->translatedFormat('F Y') }}</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-cc">
                    ...
                    <tr class="hover:cc-card transition-colors">
                        ...
                        <td class="px-5 py-3.5">
                            <div class="font-semibold text-cc">{{ $row['user']->name }}</div>
                            <div class="text-[11px] text-cc-muted capitalize">{{ $row['user']->role }}</div>
                        </td>
                        <td class="px-5 py-3.5 text-center text-cc-muted">{{ $row['total_opportunities'] }}</td>
                        ...
                        <td class="px-5 py-3.5 text-right font-bold text-cc">{{ \App\Helpers\FormatHelper::formatIDR($row['revenue'] ?? 0) }}</td>
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-2">
                                <div class="flex-1 bg-cc-card rounded-full h-2 overflow-hidden min-w-[50px]">
                                    <div class="bg-gradient-to-r from-[var(--color-secondary)] to-secondary h-full" style="width: {{ min($row['kpi_pct'] ?? 0,100) }}%"></div>
                                </div>
                                <span class="text-[11px] font-bold text-cc-muted">{{ round($row['kpi_pct'] ?? 0) }}%</span>
                            </div>
                        </td>
                    </tr>
                    @empty
